<template>
    <div class="flex gap-2 px-4 sm:py-2 overflow-x-auto w-screen md:w-auto mb-2">
        ...
        <div v-for="(item, index) in 10" :key="index" class="flex flex-col justify-center items-center font-semibold">
            <UAvatar class="border-2 border-pink-700 box-content cursor-pointer" size="xl" :src="store.profile.avatar_url" />
            <small>@{{ store.profile.username }}</small>
        </div>
    </div>
</template>
