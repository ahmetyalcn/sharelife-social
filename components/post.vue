...
const likePost = async (post) => {
    ...
    if (session?.user) {
        ...
        toast.add({ title: 'You must login to like this content', timeout: 1000, color: "blue" })
        ...
        <div class="flex items-center justify-between text-sm text-gray-600 dark:text-gray-300 mt-2">
            <div class="flex gap-2">
                ...
                <p v-else @click="likePost(post)"
                    class=" border-2 rounded-full w-auto p-2 h-8 flex border-pink-700 cursor-pointer justify-center items-center text-pink-700">
                    <i class="i-heroicons-hand-thumb-up bg-pink-700 dark:bg-pink-700  text-2xl"></i>&nbsp;{{
                        post.likes.length
                    }}
                </p>
